...
We also express our sincere thanks to \textbf{Ms. Saina S} for her support, encouragement, and guidance throughout the project.
...
\begin{tabularx}{\textwidth}{|c|*{12}{C{0.8cm}|}}
\hline
...
\begin{itemize}
    ...
    \item \textbf{Documentation:} LaTeX (XeLaTeX) for final thesis report writing.
...
Testing was carried out using a local development environment and browser clients
across desktop and mobile platforms. Backend and frontend were run independently
to measure messaging behavior, status updates, and call signaling behavior under
different network conditions.

\section{Coding Details}
\label{sec:coding-details}

\subsection{Backend Event Handling}
\label{subsec:backend-event-handling}
The backend event engine handles user connection, message delivery, typing events,
read receipts, call signaling, and reaction toggling. Event handlers are attached
to the Socket.IO server and persist transactional data through asynchronous CRUD
operations.

\begin{lstlisting}[language=Python, caption=Simplified Socket Message Flow]
@sio.event
async def send_message(sid, data):
    ...
    await send_to_user(sio, data.get("receiver_id"), "new_message", payload)
    await sio.emit("message_sent", payload, room=sid)
\end{lstlisting}

\subsection{Frontend Message Dispatch}
\label{subsec:frontend-message-dispatch}
The frontend uses optimistic rendering for better user experience and reconciles
temporary messages with server-confirmed payloads.

\begin{lstlisting}[language=JavaScript, caption=Optimistic Send (Frontend)]
const tempId = -Date.now();
addMessage({ id: tempId, content: inputText, sender: "me", status: "sending" });
socket.emit("send_message", { content: inputText, receiver_id: activeId, _tempId: tempId });
\end{lstlisting}
...
architecture and the signaling/message flow diagrams. Figure~\ref{fig:system_architecture}
shows the interaction between the user interface, API layer, real-time transport,
and persistence layer. Figure~\ref{fig:webrtc_flow} shows the signaling path
required for browser-based calls.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{system_architecture.png}
    \caption{Overall architecture of E-Chat system.}
    \label{fig:system_architecture}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{webrtc_flow.png}
    \caption{WebRTC signaling flow for audio/video calls.}
...
Functional testing was performed on authentication, contact addition, one-to-one
chat, group chat, attachments, profile updates, and call history persistence.
Event-level validation was performed for messaging and calling workflows.
...
\hline
Authentication & Signup, login, token validation, forgot-password flow & Pass \\
\hline
Messaging & Real-time send/receive, fallback HTTP send, history retrieval & Pass \\
\hline
Attachments & File upload and rendering in message thread & Pass \\
\hline
Realtime UX & Typing indicators, status updates, read receipts, reactions & Pass \\
\hline
Calling & Offer/answer/ICE signaling and call history logging & Pass (network dependent) \\
\hline
\end{tabularx}
\end{table}
...
In addition to high-level module verification, user-flow test cases were carried
out to validate the practical behavior of the system. These covered signup and
login, chat switching, optimistic message rendering, fallback HTTP messaging,
profile synchronization, and call acceptance or rejection behavior.
...
\hline
TC1 & User signs up, logs in, and receives access token for protected requests & Pass \\
\hline
TC2 & Optimistic message bubble is replaced by confirmed server message & Pass \\
\hline
TC3 & Conversation history loads correctly when the active contact changes & Pass \\
\hline
TC4 & Profile update is broadcast to connected contacts through socket events & Pass \\
\hline
TC5 & Incoming call prompt and call termination are reflected correctly in UI & Pass \\
\hline
\end{tabularx}
\end{table}
...
for usability. Figure~\ref{fig:signup_page} shows the signup page used for new user
registration, where the system collects identity and credential details before
account creation. Figure~\ref{fig:chat_window} shows the main chat window of the
application, including the conversation panel, contact list, and message area used
for real-time communication.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{signup_page.png}
    \caption{Signup Page of the E-Chat Application}
...
\begin{figure}[H]
    ...
    \caption{Chat Window of the E-Chat Application}
...
\label{tab:latency}
\begin{tabularx}{\textwidth}{|L{4.5cm}|C{3.5cm}|C{3.5cm}|}
\hline
...
A significant achievement of E-Chat is its \textbf{cost-effective operational model}.
The total monthly cost for running the system in production is summarized below:

\begin{table}[H]
\centering
\caption{Production Deployment Monthly Cost (Estimated)}
\label{tab:costs}
\begin{tabularx}{\textwidth}{|l|l|l|}
\hline
\textbf{Item} & \textbf{Service} & \textbf{Monthly Cost} \\
\hline
Virtual Private Server & VPS Basic (Ubuntu) & \rupee{}450 \\
\hline
Domain Registration & .com (prorated monthly) & \rupee{}117 \\
\hline
SSL/TLS Certificate & Let's Encrypt & \rupee{}0 \\
\hline
Object Storage (optional) & S3-compatible (S3-lite) & \rupee{}250 \\
\hline
\textbf{TOTAL} & & \textbf{\rupee{}817/month} \\
\hline
\end{tabularx}
\end{table}
...
Another important outcome is the cost-to-capability ratio of the system. The
project shows that a feature-rich messaging platform can be implemented and hosted
with moderate resources, making it appropriate for student groups, departments,
small offices, and privacy-conscious communities.
...
\hline
3 & Deployment Artifacts & Cloud deployment configurations for Railway/Fly/Koyeb/Render & Completed \\
\hline
\end{tabularx}
\end{table}
...
\hline
\texttt{Dockerfile}, \texttt{render.yaml}, \texttt{fly.toml} & Deployment configuration for different hosting targets. \\
\hline
\end{tabularx}
\end{table}
...
\begin{table}[H]
\centering
\caption{Socket.IO Events Used in E-Chat}
...
\hline
\texttt{message\_read} & Bidirectional & Updates read status of delivered messages. \\
\hline
\texttt{call\_offer} / \texttt{call\_answer} & Bidirectional & Exchanges WebRTC offer and answer descriptions. \\
\hline
\texttt{call\_ice\_candidate} & Bidirectional & Relays ICE candidate data during peer connection setup. \\
\hline
\texttt{call\_end} / \texttt{call\_reject} & Bidirectional & Handles call termination and rejection flow. \\
\hline
\texttt{message\_reaction} & Server to Client & Synchronizes message reaction updates across participants. \\
\hline
\end{tabularx}
\end{table}
